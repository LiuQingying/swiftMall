...
private let myEndpointClosure = { (target: API) -> Endpoint in
    ...
    var url:String = target.baseURL.absoluteString + target.path
    var task = target.task
    let timestamp = String.timeStamp()
    let nonce = String.random8LengthString()
    var additionalParameters = ["version":kAPPPortVersion,"timestamp":timestamp,"appid":kAppId,"nonce":nonce]
    var sign: String = String.stringWithDict(dict: additionalParameters).appending(kAppSecret)
    ...
    case .get:
        ...
        case .requestPlain:
            sign = String.stringWithDict(dict: additionalParameters).appending(kAppSecret)
        case .requestParameters(var parameters, let encoding):
            additionalParameters.forEach { parameters[$0.key] = $0.value }
            sign = String.stringWithDict(dict: parameters).appending(kAppSecret)
        default:
            break
        }
    case .post:
    ...
    additionalParameters = ["version":kAPPPortVersion,"timestamp":timestamp,"appid":kAppId,"nonce":nonce,"sign":md5Sign]
    let requestStr = String.requestTypeWithDict(dict: additionalParameters)
    url = url.appending(requestStr)
    /*
